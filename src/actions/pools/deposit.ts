...
\`\`\`json
{
    ...
    "tokenC": "EQCGScrZe1xbyWqWDvdI6mzP-GAcAWFv6ZXuaJOuSqemxku4",
    "initialLiquidity": "1"
}
\`\`\`
...
- The first token of the token pool the user wants to create
- The second token of the token pool the user wants to create
- The third token of the token pool the user wants to create
...
export class PoolCreationAction {
    ...
    async createPool(params: PoolCreatetionContent): Promise<string> {
        console.log(
            `Creating: ${params.tokenA}-${params.tokenB} pool w/ (${params.initialLiquidity}) initial liquidity`
        );
...
const buildPoolCreationDetails = async (
    ...
    let transferContent: PoolCreatetionContent = content.object as PoolCreatetionContent;
